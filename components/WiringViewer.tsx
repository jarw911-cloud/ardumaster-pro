           ...
           {visualWires.map((w, idx) => {
             ...
             return (
               <div key={idx} className="flex items-center gap-2 mb-1">
                 {isRight ? (
                    <>
                       <div className={`h-1 w-12 ${w.color} shadow-[0_0_5px_currentColor]`}></div>
                       <span className="text-[8px] font-mono text-slate-400 bg-slate-900/80 px-2 py-0.5 rounded border border-white/10">{pinLabel}</span>
                    </>
                 ) : (
                    <>
                       <span className="text-[8px] font-mono text-slate-400 bg-slate-900/80 px-2 py-0.5 rounded border border-white/10">{pinLabel}</span>
      ...
      <div className="w-[900px] lg:w-full min-h-full p-4 md:p-10 mx-auto flex flex-col items-center pb-10">
          ...
          <div className="w-full flex justify-between items-start gap-10 relative">
            ...
            <div className="flex-1 flex flex-col gap-12 items-center">
                {sensors.length === 0 ? (
                    <div className="h-40 w-full border-2 border-dashed border-white/5 rounded-[2rem] flex items-center justify-center text-slate-700 text-[10px] font-bold uppercase tracking-widest">No Input</div>
                            ...
                            <div className="mt-8 grid grid-cols-4 gap-2 w-full">
                                {usedPins.map((up, i) => (
                                    <div key={i} className="flex flex-col items-center">
                                        <div className="w-1.5 h-1.5 bg-yellow-500 rounded-full mb-1"></div>
            ...
            <div className="flex-1 flex flex-col gap-12 items-center">
                {actuators.length === 0 ? (
                    <div className="h-40 w-full border-2 border-dashed border-white/5 rounded-[2rem] flex items-center justify-center text-slate-700 text-[10px] font-bold uppercase tracking-widest">No Output</div>
